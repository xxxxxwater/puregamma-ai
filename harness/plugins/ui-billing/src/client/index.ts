...
import type { PureGammaBillingView } from '@puregamma/dsh-client-gateway'
...
const en = {
  ...
  paused: 'Paused', active: 'Active', disabled: 'Disabled', noBudgets: 'No automation budgets yet',
  rewards: 'Reward history', noRewards: 'No rewards yet', usage: 'Usage history', noUsage: 'No credit activity yet',
  action: 'Action', delta: 'Delta', balanceAfter: 'Balance after', created: 'Created', entitlements: 'Entitlements',
...
const zh: Record<keyof typeof en, string> = {
  nav: '计费', title: '访问权限与订阅', subtitle: '来自 Billing capability 的订阅、Credits、自动化预算与使用状态。',
  loading: '正在读取计费…', unavailable: '计费暂不可用', currentPlan: '当前套餐', subscription: '订阅', credits: 'Credits', periodEnd: '周期结束',
  checkoutMode: 'Checkout 模式', paymentLink: 'Primary Payment Link', configured: '已配置', notConfigured: '未配置',
  budgets: '自动化 Credits 预算', hardStop: '不足时自动暂停', automation: '自动化', thisMonth: '本月', nextEstimate: '下次预计', status: '状态',
  paused: '已暂停', active: '运行中', disabled: '已关闭', noBudgets: '暂无自动化预算',
  rewards: '奖励记录', noRewards: '暂无奖励', usage: '使用记录', noUsage: '暂无 Credit 使用记录',
  action: '操作', delta: '变化', balanceAfter: '变动后余额', created: '创建时间', entitlements: '权益',
  notificationChannels: '通知渠道', highCostTasks: '高成本研究', imessage: 'iMessage', enabled: '已启用', unavailableValue: '不可用',
  source: '数据源', observed: '观测时间', creditsPerMonth: 'Credits / 月', perMonth: ' / 月', activeBadge: '当前',
  freeTagline: '建立基础市场观察习惯。', free1: '核心资产观察', free2: '1 个组合账户连接', free3: '共享市场简报', free4: '基础事件视图', free5: 'Email 投递',
  proTagline: '适合持续跟踪个人组合的主动投资者。', pro1: '20–50 项资产', pro2: '每日组合简报', pro3: '重要事件提醒', pro4: '限量深度研究', pro5: 'Telegram / Email', pro6: '基础回测',
  maxTagline: '适合多组合与高频研究工作流。', max1: '包含全部 Pro 权益', max2: '多组合', max3: '更高 Agent 额度', max4: 'iMessage', max5: '高频监控', max6: '高级回测', max7: '私有 Playbook',
  enterpriseTagline: '适合团队、定制数据与私有部署。', enterprise1: '团队工作区', enterprise2: 'API 访问', enterprise3: '自定义数据源', enterprise4: '私有部署', enterprise5: '专属支持与合规配置',
}
...
function textRow(label: string, value: string | number | undefined, missing: string): ReactElement[] {
...
function plans(t: (key: Key) => string): Plan[] {
  return [
    { name: 'Free', price: '$0', credits: '150', tagline: t('freeTagline'), benefits: [t('free1'), t('free2'), t('free3'), t('free4'), t('free5')] },
    { name: 'Pro', price: '$29.9', credits: '3,000', tagline: t('proTagline'), benefits: [t('pro1'), t('pro2'), t('pro3'), t('pro4'), t('pro5'), t('pro6')] },
    { name: 'Max', price: '$199', credits: '15,000', tagline: t('maxTagline'), benefits: [t('max1'), t('max2'), t('max3'), t('max4'), t('max5'), t('max6'), t('max7')] },
    { name: 'Enterprise', price: 'Custom', credits: 'Custom', tagline: t('enterpriseTagline'), benefits: [t('enterprise1'), t('enterprise2'), t('enterprise3'), t('enterprise4'), t('enterprise5')] },
  ]
}
...
function BillingSection({ useResource, t }: BillingSectionProps): ReactElement {
  ...
  return createElement('section', { style: sectionStyle },
    ...
    createElement('div', { style: gridStyle },
      createElement('div', { style: cardStyle }, ...textRow(t('currentPlan'), currentPlan, t('unavailableValue'))),
      createElement('div', { style: cardStyle }, ...textRow(t('subscription'), billing.subscriptionStatus, t('unavailableValue'))),
      createElement('div', { style: cardStyle }, ...textRow(t('credits'), billing.creditBalance, t('unavailableValue'))),
      createElement('div', { style: cardStyle }, ...textRow(t('periodEnd'), billing.currentPeriodEnd, t('unavailableValue'))),
      createElement('div', { style: cardStyle }, ...textRow(t('checkoutMode'), billing.checkoutMode, t('unavailableValue'))),
      createElement('div', { style: cardStyle }, ...textRow(t('paymentLink'), billing.primaryPaymentLinkConfigured === undefined ? undefined : billing.primaryPaymentLinkConfigured ? t('configured') : t('notConfigured'), t('unavailableValue'))),
    ),
    createElement('div', { style: cardStyle },
      createElement('strong', null, t('entitlements')),
    ...
    createElement('div', { style: { ...gridStyle, gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))' } },
      ...plans(t).map((plan) => createElement('article', { key: plan.name, style: { ...cardStyle, minHeight: 250 } },
    ...
    createElement('div', { style: cardStyle },
      createElement('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' } }, createElement('strong', null, t('budgets')), createElement('span', { style: badgeStyle }, t('hardStop'))),
      billing.budgets.length ? createElement('div', { style: { overflowX: 'auto' } }, createElement('table', { style: { width: '100%', minWidth: 650, borderCollapse: 'collapse', fontSize: 12 } },
        createElement('thead', null, createElement('tr', null, ...[t('automation'), t('thisMonth'), t('nextEstimate'), t('status')].map((head) => createElement('th', { key: head, style: { textAlign: 'left', padding: '7px 8px', borderBottom: '1px solid var(--dsw-border-subtle, rgba(127,127,127,.18))' } }, head))),
        createElement('tbody', null, ...billing.budgets.map((item) => createElement('tr', { key: item.automationKey },
          createElement('td', { style: { padding: '8px' } }, item.automationKey),
          createElement('td', { style: { padding: '8px' } }, `${item.monthlyUsed ?? 0} / ${item.monthlyLimit ?? '—'}`),
          createElement('td', { style: { padding: '8px' } }, item.nextEstimatedCredits === undefined ? '—' : `${item.nextEstimatedCredits} Credits`),
          createElement('td', { style: { padding: '8px' } }, item.paused ? t('paused') : item.enabled ? t('active') : t('disabled')),
        ))),
      )) : createElement('p', { style: labelStyle }, t('noBudgets')),
    ),
    createElement('div', { style: gridStyle },
      createElement('div', { style: cardStyle }, createElement('strong', null, t('rewards')), billing.rewards.length ? createElement('div', { style: { display: 'grid', gap: 7 } }, ...billing.rewards.slice(0, 8).map((item) => createElement('div', { key: item.id, style: { display: 'flex', justifyContent: 'space-between', gap: 10, borderTop: '1px solid var(--dsw-border-subtle, rgba(127,127,127,.16))', paddingTop: 7, fontSize: 12 } }, createElement('span', null, item.rewardType ?? 'Reward', createElement('span', { style: labelStyle }, item.createdAt ? ` · ${item.createdAt}` : '')), createElement('strong', null, item.credits === undefined ? '—' : `+${item.credits}`))) : createElement('p', { style: labelStyle }, t('noRewards'))),
      createElement('div', { style: cardStyle }, createElement('strong', null, t('usage')), billing.usageHistory.length ? createElement('div', { style: { maxHeight: 320, overflowY: 'auto' } }, createElement('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: 12 } },
        createElement('thead', null, createElement('tr', null, ...[t('action'), t('delta'), t('balanceAfter'), t('created')].map((head) => createElement('th', { key: head, style: { textAlign: 'left', padding: '7px 6px', borderBottom: '1px solid var(--dsw-border-subtle, rgba(127,127,127,.18))' } }, head))),
        createElement('tbody', null, ...billing.usageHistory.slice(0, 40).map((item, index) => createElement('tr', { key: item.id ?? `${item.createdAt ?? 'usage'}-${index}` }, createElement('td', { style: { padding: '7px 6px' } }, item.action ?? '—'), createElement('td', { style: { padding: '7px 6px' } }, item.creditsDelta ?? '—'), createElement('td', { style: { padding: '7px 6px' } }, item.balanceAfter ?? '—'), createElement('td', { style: { padding: '7px 6px' } }, item.createdAt ?? '—')))),
      )) : createElement('p', { style: labelStyle }, t('noUsage'))),
    ),
