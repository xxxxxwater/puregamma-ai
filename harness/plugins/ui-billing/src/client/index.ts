...
const en = {
  ...
  checkoutMode: 'Checkout mode', paymentLink: 'Primary Payment Link', configured: 'Configured', notConfigured: 'Not configured', budgets: 'Automation credit budgets',
  hardStop: 'Hard stop when exhausted', automation: 'Automation', thisMonth: 'This month', nextEstimate: 'Next estimate', status: 'Status', paused: 'Paused', active: 'Active', disabled: 'Disabled',
  noBudgets: 'No automation budgets yet', rewards: 'Reward history', noRewards: 'No rewards yet', usage: 'Usage history', noUsage: 'No credit activity yet', action: 'Action', delta: 'Delta', balanceAfter: 'Balance after', created: 'Created', entitlements: 'Entitlements',
  notificationChannels: 'Notification channels', highCostTasks: 'High-cost research', imessage: 'iMessage', enabled: 'Enabled', unavailableValue: 'Unavailable', source: 'Source', observed: 'Observed', creditsPerMonth: 'credits / month', perMonth: ' / month', activeBadge: 'Active',
  checkout: 'Choose plan', paymentCheckout: 'Payment link', portal: 'Open billing portal', cancel: 'Cancel at period end', reactivate: 'Reactivate', working: 'Working…', actionFailed: 'Action unavailable', actionReady: 'Action completed. Billing state will refresh automatically.',
  confirmCancel: 'Cancel this subscription at the end of the current billing period?',
...
const zh: Record<keyof typeof en, string> = {
  ...
  checkoutMode: 'Checkout 模式', paymentLink: 'Primary Payment Link', configured: '已配置', notConfigured: '未配置', budgets: '自动化 Credits 预算', hardStop: '不足时自动暂停', automation: '自动化', thisMonth: '本月', nextEstimate: '下次预计', status: '状态', paused: '已暂停', active: '运行中', disabled: '已关闭',
  noBudgets: '暂无自动化预算', rewards: '奖励记录', noRewards: '暂无奖励', usage: '使用记录', noUsage: '暂无 Credit 使用记录', action: '操作', delta: '变化', balanceAfter: '变动后余额', created: '创建时间', entitlements: '权益', notificationChannels: '通知渠道', highCostTasks: '高成本研究', imessage: 'iMessage', enabled: '已启用', unavailableValue: '不可用', source: '数据源', observed: '观测时间', creditsPerMonth: 'Credits / 月', perMonth: ' / 月', activeBadge: '当前',
  checkout: '选择套餐', paymentCheckout: 'Payment Link', portal: '打开计费门户', cancel: '周期结束时取消', reactivate: '恢复订阅', working: '处理中…', actionFailed: '操作不可用', actionReady: '操作完成，Billing 状态会自动刷新。', confirmCancel: '确定要在当前计费周期结束时取消订阅吗？',
...
function plans(t: (key: Key) => string): Array<{ name: string; price: string; credits: string; tagline: string; benefits: string[] }> {
  const raw = [
    ['Free', '$0', '150', 'freeTagline', ['free1', 'free2', 'free3', 'free4', 'free5']],
    ['Pro', '$29.9', '3,000', 'proTagline', ['pro1', 'pro2', 'pro3', 'pro4', 'pro5', 'pro6']],
    ['Max', '$199', '15,000', 'maxTagline', ['max1', 'max2', 'max3', 'max4', 'max5', 'max6', 'max7']],
    ['Enterprise', 'Custom', 'Custom', 'enterpriseTagline', ['enterprise1', 'enterprise2', 'enterprise3', 'enterprise4', 'enterprise5']],
  ] as const
...
function budgetTable(budgets: PureGammaBillingBudget[], t: (key: Key) => string): ReactElement {
  ...
  return createElement('div', { style: { overflowX: 'auto' } }, createElement('table', { style: { width: '100%', minWidth: 650, borderCollapse: 'collapse', fontSize: 12 } },
...
function BillingSection({ useResource, t, remote }: BillingSectionProps): ReactElement {
  ...
  const planCards = plans(t).map((plan) => createElement('article', { key: plan.name, style: { ...cardStyle, minHeight: 250 } },
    ...
    currentPlan !== plan.name && plan.name !== 'Free' ? createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 8 } },
      createElement(ActionButton, { label: t('checkout'), busy: busy === `checkout:${plan.name}`, onClick: () => run(`checkout:${plan.name}`, () => remote.billingCheckout(plan.name)) }),
      billing.primaryPaymentLinkConfigured === true ? createElement(ActionButton, { label: t('paymentCheckout'), busy: busy === `payment:${plan.name}`, onClick: () => run(`payment:${plan.name}`, () => remote.billingPaymentLinkCheckout(plan.name)) }) : null,
    ) : null,
  ))

  const subscriptionAction = billing.cancelAtPeriodEnd === true
    ? createElement(ActionButton, { label: t('reactivate'), busy: busy === 'reactivate', onClick: () => run('reactivate', () => remote.billingReactivate(), false) })
    : createElement(ActionButton, { label: t('cancel'), busy: busy === 'cancel', danger: true, onClick: async () => { if (window.confirm(t('confirmCancel'))) await run('cancel', () => remote.billingCancel(), false) } })
  ...
  return createElement('section', { style: sectionStyle },
    ...
    createElement('div', { style: { ...cardStyle, display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' } },
      createElement('div', { style: { display: 'grid', gap: 5 } }, createElement('strong', null, t('subscription')), createElement('span', { style: labelStyle }, billing.cancelAtPeriodEnd ? t('cancel') : t('active'))),
      subscriptionAction,
      createElement(ActionButton, { label: t('portal'), busy: busy === 'portal', onClick: () => run('portal', () => remote.billingPortal()) }),
    ),
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.slots.inject('settings.section', () => ctx.slots.register({
    name: 'settings.section', id: 'puregamma-billing', order: 30, label: () => t('nav'), inject: () => ({ t, remote: ctx.remote.puregammaClient }),
  }, BillingSection))
}
