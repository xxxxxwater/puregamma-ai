...
function normalizeLocale(value: SecretaryLocale | undefined): SecretaryLocale {
...
export class LegacyApiSecretaryProvider extends SecretaryService {
  ...
  private url(path: string, query: Record<string, string | undefined> = {}): URL {
  ...
  async state(locale: SecretaryLocale = 'zh'): Promise<SecretaryState> {
    const response = await this.fetchOwned(`/secretary?locale=${encodeURIComponent(normalizeLocale(locale))}`, {
      method: 'GET',
      headers: { Accept: 'application/json', Authorization: `Bearer ${this.token()}` },
    })
    if (!response.ok) throw new Error(`secretary compatibility API returned HTTP ${response.status} for state`)
    const payload = await response.json()
    if (!isRecord(payload)) throw new Error('secretary compatibility API returned invalid state')
    ...
    return {
      ...
      voice: {
        id: stringValue(voice.id, 'unknown'),
        name: stringValue(voice.name, 'unknown'),
        fixed: boolValue(voice.fixed),
      },
      skills,
      memory: {
        enabled: boolValue(memory.enabled),
        isolatedByUser: boolValue(memory.isolated_by_user),
      },
      billing: {
        creditsPerReply: finiteNumber(billing.credits_per_reply),
        creditBalance: finiteNumber(billing.credit_balance),
      },
    }
  }
  ...
  async synthesizeVoice(request: SecretaryVoiceRequest): Promise<SecretaryAudio> {
    ...
    const response = await this.fetchOwned('/secretary/voice', {
      method: 'POST',
      headers: {
        Accept: 'audio/*',
        Authorization: `Bearer ${this.token()}`,
  ...
  async transcribe(audio: Uint8Array, contentType: string, locale: SecretaryLocale = 'zh'): Promise<SecretaryTranscription> {
    ...
    const response = await this.fetchOwned(`/secretary/transcribe?locale=${encodeURIComponent(normalizeLocale(locale))}`, {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        Authorization: `Bearer ${this.token()}`,
