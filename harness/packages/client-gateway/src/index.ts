...
function billingAction(document: { observedAt: string; source: string; payload: Record<string, unknown> }, kind: PureGammaBillingActionResult['kind'], planName?: string): PureGammaBillingActionResult {
  const payload = document.payload
  const url = stringValue(payload, 'url') ?? stringValue(payload, 'checkout_url') ?? stringValue(payload, 'portal_url') ?? stringValue(payload, 'session_url') ?? stringValue(payload, 'payment_url')
  return { available: true, observedAt: document.observedAt, source: document.source, kind, ...(planName === undefined ? {} : { planName }), status: stringValue(payload, 'status') ?? stringValue(payload, 'subscription_status'), ...(url === undefined ? {} : { url }) }
}
...
function notificationAction(document: { observedAt: string; source: string; payload: Record<string, unknown> }, kind: PureGammaNotificationActionResult['kind']): PureGammaNotificationActionResult {
  const payload = document.payload, delivery = record(payload.delivery), verification = record(payload.verification)
  return { available: true, observedAt: document.observedAt, source: document.source, kind, challengeId: stringValue(payload, 'challenge_id') ?? stringValue(verification, 'challenge_id'), recipient: stringValue(payload, 'recipient'), status: stringValue(payload, 'status') ?? stringValue(delivery, 'status'), deliveryId: stringValue(payload, 'delivery_id') ?? stringValue(delivery, 'id') }
}
...
function notificationDeliveries(payload: Record<string, unknown> | undefined): PureGammaNotificationDelivery[] {
  return objectArray(payload, 'deliveries').map((item) => ({ id: stringValue(item, 'id'), channel: stringValue(item, 'channel'), status: stringValue(item, 'status'), messagePreview: stringValue(item, 'message_preview') ?? stringValue(item, 'message'), provider: stringValue(item, 'provider'), error: stringValue(item, 'error'), createdAt: stringValue(item, 'created_at'), sentAt: stringValue(item, 'sent_at') }))
}
...
export class PureGammaClientGateway extends TypertRemoteService {
  ...
  async notifications(): Promise<PureGammaNotificationsView> {
    ...
    try {
      const [dailyBrief, deliveries, imessageConfig, devices] = await Promise.all([service.dailyBrief(), service.deliveries(), service.imessageConfig(), service.devices()])
      const dailyPayload = record(dailyBrief.payload), preference = record(dailyPayload?.preference), deliveryRows = notificationDeliveries(record(deliveries.payload)), latest = deliveryRows[0], imessage = record(imessageConfig.payload), devicePayload = record(devices.payload)
      return { available: true, observedAt: new Date().toISOString(), source: dailyBrief.source, enabled: booleanValue(preference, 'enabled'), timezone: stringValue(preference, 'timezone'), localTime: stringValue(preference, 'local_time'), channel: stringValue(preference, 'channel'), channels: stringArray(preference, 'channels'), reportTypes: stringArray(preference, 'report_types'), locale: stringValue(preference, 'locale'), includePortfolio: booleanValue(preference, 'include_portfolio'), includeMarket: booleanValue(preference, 'include_market'), includeSignals: booleanValue(preference, 'include_signals'), includeRisk: booleanValue(preference, 'include_risk'), includeSentiment: booleanValue(preference, 'include_sentiment'), maxLength: numberValue(preference, 'max_length'), quietHours: record(preference?.quiet_hours) as PureGammaNotificationsView['quietHours'], failureCount: numberValue(preference, 'failure_count'), lastError: stringValue(preference, 'last_error'), nextDeliveryAt: stringValue(preference, 'next_delivery_at'), recentDeliveries: deliveryRows.length, lastDeliveryStatus: stringValue(latest, 'status'), lastDeliveryChannel: stringValue(latest, 'channel'), lastDeliveryAt: stringValue(latest, 'sent_at') ?? stringValue(latest, 'created_at'), deliveryAvailable: booleanValue(devicePayload, 'delivery_available'), imessage: imessage === undefined ? undefined : { officialNumber: stringValue(imessage, 'official_number'), provider: stringValue(imessage, 'provider'), enabledPlans: stringArray(imessage, 'enabled_plans'), recipient: stringValue(imessage, 'recipient'), recipientVerifiedAt: stringValue(imessage, 'recipient_verified_at') }, deliveries: deliveryRows }
    } catch { return unavailable<PureGammaNotificationsView>('cordis:pgNotifications', 'notification data is currently unavailable') }
  }
  ...
  async notificationsTestEmail(): Promise<PureGammaNotificationActionResult> { const service = this.ctx.get('pgNotifications'); if (service === undefined) return notificationActionUnavailable('email-test', 'notifications capability is not installed'); try { return notificationAction(await service.send({ channel: 'email', message: 'PureGamma AI test notification.', idempotencyKey: `ui-test:${randomUUID()}` }), 'email-test') } catch { return notificationActionUnavailable('email-test', 'email test delivery is currently unavailable') } }
