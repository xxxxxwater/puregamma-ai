...
def build_scheduler() -> BlockingScheduler:
    ...
    # General NAV refresh for every connected portfolio, independent of the
    # Autopilot-gated sync above, so NAV snapshots stay fresh for all users.
    scheduler.add_job(
        enqueue,
        IntervalTrigger(minutes=15),
        args=["puregamma.sync_all_portfolio_accounts"],
        id="portfolio_nav_sync",
        max_instances=1,
        coalesce=True,
    )
    scheduler.add_job(
