...
SKILL_TOOL_ALLOWLISTS: dict[str, set[str]] = {
    ...
    "deep_research": {"get_market_quote", "get_market_history", "search_source_documents", "search_online_sources", "get_defi_protocol_metrics", "get_chain_metrics", "get_data_source_status", "get_account_snapshot", "get_position_snapshot", "get_options_context", "list_research_strategies", "run_backtest", "get_strategy_performance"},
}

STRATEGY_AUTOMATION_TOOL_ALLOWLIST = {
    "create_strategy_draft",
    "modify_strategy_draft",
    "run_backtest",
    "save_backtest_as_draft",
...
class AgentToolRegistry:
    ...
    def __init__(self, db: Session, user_id: str, conversation_id: str | None = None):
        ...
        self.tools: dict[str, Callable[..., ToolResult]] = {
            ...
            "save_backtest_as_draft": self.save_backtest_as_draft,
    ...
    def plan(self, query: str, skills: list[str] | None = None, data_sources: list[str] | None = None, skill_tool_allowlist: set[str] | None = None) -> list[tuple[str, dict[str, Any]]]:
        ...
        if strategy_words and any(word in lowered for word in ("backtest", "回测")):
            return permitted([("run_backtest", {"symbols": symbols or ["BTC"]})])
    ...
    def run_backtest(
        self, symbols: list[str] | None = None, lookback_days: int = 90
    ) -> ToolResult:
        result = self.run_nautilus_backtest(symbols or ["BTC"], lookback_days)
        return ToolResult(
            "run_backtest",
            result.data,
            result.summary,
    ...
    def save_backtest_as_draft(self, run_id: str) -> ToolResult:
        from apps.api.services.unified_backtest_service import (
            save_run_as_strategy,
            serialize_saved_strategy,
        )

        payload = save_run_as_strategy(self.db, self.user_id, run_id)
        return ToolResult(
            "save_backtest_as_draft",
            serialize_saved_strategy(payload),
            "Saved completed backtest as a strategy draft; no runtime was started",
        )
    ...
    def create_paper_run(self) -> ToolResult:
        raise PermissionError("Agent tools may only preview PAPER activation")

    def create_shadow_run(self) -> ToolResult:
        raise PermissionError("Agent tools may only preview SHADOW activation")
    ...
    def preview_strategy_activation(self, mode: str = "PAPER") -> ToolResult:
        ...
        strategy = self._latest_strategy()
        intent, _confirmation = preview_activation(
            self.db,
            self.user_id,
            strategy.id,
            mode=mode,
    ...
    def activate_strategy_tool(self, confirmation: str) -> ToolResult:
        raise PermissionError("Agent tools cannot activate strategies")
    ...
    def resume_strategy(self) -> ToolResult:
        raise PermissionError("Agent tools cannot resume strategies")

    def stop_strategy(self) -> ToolResult:
        return self._transition_strategy("stop")

    def reconcile_account_tool(self) -> ToolResult:
        raise PermissionError("Agent tools cannot reconcile accounts")

    def generate_order_preview(self, request: str, symbols: list[str]) -> ToolResult:
        raise PermissionError("Agent tools cannot generate manual order previews")
