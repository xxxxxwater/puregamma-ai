...
from packages.database.models import BacktestArtifact, BacktestRun, StrategyRiskPolicy, StrategyVersion, TradingStrategy
...
def save_run_as_strategy(db: Session, user_id: str, run_id: str) -> dict:
    ...
    spec = parse_spec((row.spec_json or {}).get("spec") or row.params_json).model_dump()
    try:
        executable = compile_backtest_spec(
            spec,
            assumptions=row.assumptions_json or {},
            result=row.result_json or {},
            window_days=(row.spec_json or {}).get("window_days"),
            execution_mode="PAPER",
            version=1,
            created_by=user_id,
        )
    except StrategyCompilationError as exc:
        raise ValueError(f"backtest spec cannot be compiled for runtime: {exc}") from exc
    draft = {
