...
def _account_quality(account: TradingAccount, snapshot: AccountSnapshot | None) -> dict:
    if not snapshot:
        return {
            "id": account.id,
    ...
    stale = _snapshot_is_stale(account, snapshot)
    return {
        "id": account.id,
        "provider": account.venue.lower(),
        "name": account.name,
...
def _portfolio_data_quality(accounts: list[TradingAccount], snapshots_by_account: dict[str, AccountSnapshot]) -> dict:
    missing = []
    stale = []
    current = []
    for account in accounts:
        row = _account_quality(account, snapshots_by_account.get(account.id))
    ...
    if not accounts:
        status = "NO_ACCOUNTS"
    elif missing and not (current or stale):
        status = "PENDING_SYNC"
    elif missing:
        status = "PARTIAL"
    elif stale:
    ...
    return {
        ...
        "synced_accounts": len(current) + len(stale),
        "source_count": len({account.venue.lower() for account in accounts}),
...
def portfolio_view(db: Session, user: User) -> dict:
    ...
    latest_snapshot_by_account = {row.account_id: row for row in latest}
    timeline = []
    latest_by_account = {}
    ...
    for account in accounts:
        connection = _connection(db, account)
        snapshot = latest_snapshot_by_account.get(account.id)
        if snapshot and _snapshot_is_stale(account, snapshot):
            effective_status = "STALE"
        elif not snapshot and connection.status == "CONNECTED":
            effective_status = "PENDING_SYNC"
        else:
    ...
    data_quality = _portfolio_data_quality(accounts, latest_snapshot_by_account)
    return {
        "connected": bool(latest),
        "partial": bool(data_quality["missing_accounts"]),
        "stale": bool(data_quality["stale_accounts"]),
...
def portfolio_context(db: Session, user_id: str, *, detailed: bool = False) -> dict:
    ...
    latest_snapshot_by_account: dict[str, AccountSnapshot] = {}
    missing: list[str] = []
    stale = False
    for account in accounts:
        ...
        latest_snapshot_by_account[account.id] = snapshot
        stale = stale or _snapshot_is_stale(account, snapshot)
        if _snapshot_is_stale(account, snapshot):
            missing.append(f"{account.name}: snapshot is stale")
    nav = sum(float(row.equity) for row in latest_snapshots)
