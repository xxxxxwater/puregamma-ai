...
from packages.trading.schemas.strategy_specs import ensure_executable_strategy
...
def validate_draft(draft: dict) -> dict:
    ...
    try:
        executable = ensure_executable_strategy(parsed.model_dump(mode="json"))
    except Exception as exc:
        runtime_ready = False
        runtime_errors.append(str(exc))
        warnings.append(f"Strategy is not runtime executable yet: {exc}")
    return {
        "valid": not errors,
        "errors": errors,
        "warnings": warnings,
        ...
        "draft": executable or parsed.model_dump(mode="json"),
    }
...
def create_strategy(
    ...
    *,
    ...
    draft.version = 1
    payload = ensure_executable_strategy(draft.model_dump(mode="json"))
...
def preview_activation(
    ...
    *,
    ...
    try:
        executable_strategy = ensure_executable_strategy(version.draft_json)
    except Exception as exc:
        raise StrategyControlError(
            f"Strategy version is not executable: {exc}"
        ) from exc
    ...
    if not risk_row:
        risk = executable_strategy["risk_policy"]
        risk_row = StrategyRiskPolicy(
            user_id=user_id,
            strategy_id=strategy.id,
            strategy_version=version.version,
            max_position=float(risk["max_position"]),
            ...
            reduce_only=bool(risk.get("reduce_only", False)),
            pause_opening=bool(risk.get("pause_opening", False)),
            global_kill_switch=bool(risk.get("global_kill_switch", False)),
            policy_json=risk,
        )
        db.add(risk_row)
        db.flush()
    risk_payload = serialize_risk_policy(risk_row)
    intent = StrategyIntent(
        ...
        payload_json={
            ...
            "risk": risk_payload,
            "preview": _activation_preview_summary(
                strategy, version, executable_strategy, risk_payload, resolved_mode.value
            ),
        },
...
def activate_strategy(
    ...
    *,
    ...
    payload = {
        ...
        "strategy": intent.payload_json.get("strategy") or version.draft_json,
...
def _activation_preview_summary(
    strategy: TradingStrategy,
    version: StrategyVersion,
    executable_strategy: dict,
    risk: dict,
    mode: str,
) -> dict:
    sizing = executable_strategy.get("position_sizing") or {}
    backtest_config = executable_strategy.get("backtest_config") or {}
    return {
        "strategy_name": strategy.name,
        "strategy_id": strategy.id,
        "strategy_version": version.version,
        "execution_mode": mode,
        "instruments": executable_strategy.get("instruments", []),
        "venues": executable_strategy.get("venues", []),
        "timeframe": executable_strategy.get("timeframe"),
        "strategy_type": executable_strategy.get("strategy_type"),
        "strategy_subtype": executable_strategy.get("strategy_subtype"),
        "direction": sizing.get("direction", "LONG_ONLY"),
        "max_position": risk.get("max_position"),
        "max_notional": risk.get("max_notional"),
        "leverage": risk.get("max_leverage"),
        "stop_loss": executable_strategy.get("stop_loss"),
        "max_daily_loss": risk.get("max_daily_loss"),
        "max_drawdown": risk.get("max_drawdown"),
        "estimated_fee_bps": backtest_config.get("fee_bps"),
        "estimated_slippage_bps": backtest_config.get("slippage_bps"),
        "output_contract": executable_strategy.get("output_contract"),
        "expires_minutes": 15,
    }
