...
def email_login(
    ...
    credentials_valid = bool(user) and bool(user.password_hash) and verify_password(payload.password, user.password_hash)

    if not credentials_valid:
        # Captcha is only required after the first failed attempt, so genuine
        # users are not blocked before their credentials are even checked.
        # The rate limit above still bounds brute-force probing.
        if not payload.captcha_id or payload.captcha_offset is None:
            raise HTTPException(status_code=401, detail={"code": "CAPTCHA_REQUIRED"})
        try:
            verify_captcha(payload.captcha_id, payload.captcha_offset)
        except CaptchaError as exc:
            raise HTTPException(status_code=400, detail={"code": exc.code}) from exc
