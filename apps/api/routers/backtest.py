...
def create(payload: BacktestRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> dict:
    try:
        return {"backtest": serialize_run(run_backtest(db, user.id, payload.strategy_name, payload.asset, payload.params, engine=payload.engine, strategy_id=payload.strategy_id, idempotency_key=payload.idempotency_key))}
