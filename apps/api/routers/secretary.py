...
@router.post("/voice")
def create_voice(payload: VoiceRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> Response:
    api_key = os.getenv("NOIZ_API_KEY", "").strip()
    if not api_key:
        raise HTTPException(status_code=503, detail={"code": "NOIZ_NOT_CONFIGURED"})
    text = payload.text.strip()
    if not text:
        raise HTTPException(status_code=400, detail={"code": "EMPTY_TEXT"})
    quote = quote_task(task_type="secretary_voice", requested_model="default", input_tokens=max(1, len(text) // 4))
    try:
        reservation = reserve_task(db, user.id, quote, f"secretary-voice:{user.id}:{uuid.uuid4()}", {"surface": "secretary", "action": "voice"})
        db.commit()
    except InsufficientCreditsError as exc:
        db.rollback()
        raise HTTPException(status_code=402, detail={"code": "INSUFFICIENT_CREDITS", "required": quote.credits}) from exc
...
def transcribe_voice(
    ...
    try:
        reservation = reserve_task(db, user.id, quote, f"secretary-transcribe:{user.id}:{uuid.uuid4()}", {"surface": "secretary", "action": "transcribe"})
        db.commit()
    except InsufficientCreditsError as exc:
        db.rollback()
        raise HTTPException(status_code=402, detail={"code": "INSUFFICIENT_CREDITS", "required": quote.credits}) from exc
    ...
    settle_task(db, user.id, reservation, quote.credits, metadata={"surface": "secretary", "action": "transcribe"})
    db.commit()
