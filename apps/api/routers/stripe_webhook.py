...
async def stripe_webhook(
    ...
    try:
        if settings.billing_mode == "stripe":
            ...
            # Non-Stripe billing modes must never process unsigned webhook
            # payloads: a forged checkout.session.completed could otherwise
            # grant paid plans and credits in dev/staging environments.
            raise HTTPException(status_code=503, detail="Stripe webhook is disabled in this billing mode")
