  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <ResearchCard><h2 className="mb-3 font-semibold">{copy.modules.users}</h2><div className="space-y-2">{users.users.map((user, index) => <div key={user.id} className="grid gap-3 border border-border-pg bg-bg-panel-muted p-3 text-sm md:grid-cols-[42px_1fr_auto]"><span className="text-text-pg-dim">{String(index + 1).padStart(2, "0")}</span><span>{maskEmail(user.email)}</span><div className="flex gap-2"><Badge>{user.role}</Badge><Badge tone="neutral">{user.plan}</Badge></div></div>)}</div></ResearchCard>
        <ResearchCard><h2 className="mb-3 font-semibold">{copy.modules.subscriptions}</h2><div className="grid gap-2 text-sm"><div className="border border-border-pg bg-bg-panel-muted p-3"><Badge tone="neutral"><StatusDot tone="amber" /> {copy.details.mockSubscriptionMirror}</Badge><p className="mt-2 text-text-pg-muted">{copy.details.subscriptionRecords}</p></div><div className="border border-border-pg bg-bg-panel-muted p-3 text-text-pg-muted">{copy.details.sensitiveMasked}</div></div></ResearchCard>
      </div>
      <div className="grid gap-4 xl:grid-cols-3">
        <ResearchCard><h2 className="mb-3 font-semibold">{copy.modules.dataSourceStatus}</h2><div className="space-y-2">{dataSources.sources.slice(0, 8).map((source) => <div key={source.source} className="flex items-center justify-between border border-border-pg bg-bg-panel-muted p-2 text-sm"><span>{source.source}</span><DataSourceStatusBadge locale={locale} status={source.status} /></div>)}</div></ResearchCard>
        <ResearchCard><h2 className="mb-3 font-semibold">{copy.modules.stripeWebhookEvents}</h2><div className="space-y-2">{stripeEvents.stripe_events.length ? stripeEvents.stripe_events.map((event) => <div key={event.id} className="grid gap-2 border border-border-pg bg-bg-panel-muted p-2 text-sm"><div className="flex items-center justify-between"><span>{event.event_type}</span><Badge tone="neutral"><StatusDot tone={event.requires_manual_review ? "amber" : event.processed ? "emerald" : "amber"} /> {event.requires_manual_review ? "manual review" : event.processed ? "processed" : "pending"}</Badge></div>{event.error_message ? <div className="text-xs text-status-warning">{event.error_message}</div> : null}</div>) : <span className="text-sm text-text-pg-muted">{copy.details.noWebhookEvents}</span>}</div></ResearchCard>
        <ResearchCard><h2 className="mb-3 font-semibold">{copy.modules.notificationDeliveries}</h2><div className="space-y-2">{notifications.notifications.length ? notifications.notifications.map((item) => <div key={item.id} className="flex items-center justify-between border border-border-pg bg-bg-panel-muted p-2 text-sm"><span>{item.channel}</span><Badge tone="neutral"><StatusDot tone={item.status === "sent" ? "emerald" : "amber"} /> {item.status}</Badge></div>) : <span className="text-sm text-text-pg-muted">{copy.details.noDeliveries}</span>}</div></ResearchCard>
      </div>
      <div className="grid gap-4 xl:grid-cols-2">
        <ResearchCard><h2 className="mb-3 font-semibold">BillingCheckoutIntent</h2><div className="space-y-2">{billingIntents.billing_intents.length ? billingIntents.billing_intents.slice(0, 8).map((intent) => <div key={intent.id} className="grid gap-2 border border-border-pg bg-bg-panel-muted p-2 text-sm"><div className="flex items-center justify-between"><span>{intent.public_reference}</span><Badge tone="neutral"><StatusDot tone={intent.status === "requires_manual_review" ? "amber" : intent.status === "completed" ? "emerald" : "neutral"} /> {intent.status}</Badge></div><div className="text-xs text-text-pg-muted">{intent.checkout_mode} / {intent.plan_name}</div></div>) : <span className="text-sm text-text-pg-muted">No billing intents in mock state.</span>}</div></ResearchCard>
        <ResearchCard><h2 className="mb-3 font-semibold">LLM Provider Status</h2><div className="grid gap-2 text-sm"><div className="flex items-center justify-between border border-border-pg bg-bg-panel-muted p-2"><span>{llmStatus.provider}</span><Badge tone="neutral"><StatusDot tone={llmStatus.status === "healthy" ? "emerald" : "amber"} /> {llmStatus.status}</Badge></div><div className="border border-border-pg bg-bg-panel-muted p-2 text-text-pg-muted">active={llmStatus.active_provider} / model={llmStatus.model}</div>{llmStatus.last_error ? <div className="text-xs text-status-warning">{llmStatus.last_error}</div> : null}</div></ResearchCard>
      </div>
