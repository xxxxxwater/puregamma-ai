...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      // First attempt is sent without a captcha; the server only asks for one
      // after a failed login (CAPTCHA_REQUIRED), so valid users are never
      // blocked by the puzzle before their credentials are checked.
      ...
      if (apiError.code === "CAPTCHA_REQUIRED" || apiError.code === "CAPTCHA_EXPIRED" || apiError.code === "CAPTCHA_UNAVAILABLE") {
        openCaptcha();
