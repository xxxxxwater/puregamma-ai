...
export default function AccountPage() {
  ...
  return <div className="space-y-5">
    ...
    {error ? <div className="border border-status-negative p-4 text-sm text-status-negative rounded-xl">{error}</div> : null}
    ...
    {user ? <div className="grid gap-4 lg:grid-cols-2">
      <ResearchCard><div className="flex items-start gap-4">{user.avatar_url ? <span aria-label={user.name} className="h-14 w-14 shrink-0 rounded-full border border-border-pg bg-cover bg-center" style={{ backgroundImage: `url(${user.avatar_url})` }} /> : <div className="grid h-14 w-14 place-items-center rounded-full border border-border-pg"><UserRound className="h-6 w-6" /></div>}<div className="min-w-0"><h2 className="truncate text-lg font-semibold">{user.name}</h2><p className="truncate text-sm text-text-pg-muted">{user.email}</p><div className="mt-3 flex flex-wrap gap-2">{user.login_methods?.map((method) => <span key={method} className="inline-flex items-center gap-1 border border-border-pg px-2 py-1 text-xs rounded-lg">{method === "google" ? <Chrome className="h-3 w-3" /> : null}{method === "email" ? <Key className="h-3 w-3" /> : null}{method}</span>)}{user.email_verified ? <span className="inline-flex items-center gap-1 border border-border-pg px-2 py-1 text-xs text-status-positive rounded-lg"><BadgeCheck className="h-3 w-3" />{zh ? "邮箱已验证" : "Email verified"}</span> : null}{user.has_password ? <span className="inline-flex items-center gap-1 border border-border-pg px-2 py-1 text-xs rounded-lg"><Key className="h-3 w-3" />{zh ? "已设密码" : "Password set"}</span> : null}</div></div></div><dl className="mt-5 grid grid-cols-2 gap-px border border-border-pg bg-border-pg text-sm rounded-xl overflow-hidden"><div className="bg-bg-panel p-3"><dt className="text-text-pg-muted">{zh ? "最近登录" : "Last login"}</dt><dd className="mt-1">{user.last_login_at ? new Date(user.last_login_at).toLocaleString(locale) : "-"}</dd></div><div className="bg-bg-panel p-3"><dt className="text-text-pg-muted">{zh ? "当前套餐" : "Current plan"}</dt><dd className="mt-1 font-semibold">{user.plan}</dd></div></dl></ResearchCard>
      <ResearchCard><h2 className="font-semibold">Agent</h2><div className="mt-4 grid grid-cols-3 gap-px border border-border-pg bg-border-pg text-center rounded-xl overflow-hidden"><div className="bg-bg-panel p-3"><div className="text-xs text-text-pg-muted">{zh ? "已用" : "Used"}</div><div className="mt-1 text-xl font-semibold">{quota?.used ?? 0}</div></div><div className="bg-bg-panel p-3"><div className="text-xs text-text-pg-muted">{zh ? "剩余" : "Remaining"}</div><div className="mt-1 text-xl font-semibold">{quota?.remaining ?? 0}</div></div><div className="bg-bg-panel p-3"><div className="text-xs text-text-pg-muted">Credits</div><div className="mt-1 text-xl font-semibold">{quota?.credit_balance ?? user.credit_balance}</div></div></div><button type="button" onClick={signOut} className="mt-5 inline-flex items-center gap-2 border border-border-pg px-3 py-2 text-sm hover:border-border-pg-strong rounded-lg"><LogOut className="h-4 w-4" />{zh ? "退出登录" : "Sign out"}</button></ResearchCard>
    </div> : null}
