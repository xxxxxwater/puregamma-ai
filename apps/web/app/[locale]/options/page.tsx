...
export default async function OptionsPage({ params, searchParams }: { params: { locale: Locale }; searchParams: { currency?: string } }) {
  ...
  return <div className="space-y-5">
    ...
    <ResearchCard>
      <div className="mb-4 flex items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 font-semibold"><TrendingUp className="h-4 w-4" />Long Gamma {zh ? "美股候选" : "US Stock Candidates"} — {zh ? "财报季" : "Earnings"}</h2><p className="mt-2 text-sm text-text-pg-muted">{zh ? "未来一周公布财报的美股标的，综合标的情绪、新闻覆盖与波动性排序。每2日自动刷新。" : "US stocks with upcoming earnings in the next week, ranked by sentiment, news coverage, and volatility. Auto-refreshed every 2 days."}</p></div><ShieldCheck className="h-5 w-5 text-status-positive" /></div>
      <div className="overflow-x-auto"><table className="w-full min-w-[800px] text-sm"><thead><tr className="border-y border-border-pg text-left text-xs text-text-pg-muted">{[zh ? "标的" : "Symbol", zh ? "名称" : "Name", zh ? "评分" : "Score", zh ? "行业" : "Sector", zh ? "市值" : "Cap", zh ? "财报日期" : "Earnings", (zh ? "关联新闻" : "News")].map((item) => <th key={item} className="px-3 py-2 font-medium">{item}</th>)}</tr></thead><tbody>{earnings.candidates.map((item) => <tr key={item.symbol} className="border-b border-border-pg"><td className="px-3 py-3 font-semibold">{item.symbol}</td><td className="px-3 py-3 text-text-pg-muted">{item.name}</td><td className="px-3 py-3">{item.research_score.toFixed(1)}</td><td className="px-3 py-3">{item.sector}</td><td className="px-3 py-3">{item.market_cap_category}</td><td className="px-3 py-3">{item.earnings_date || "-"}</td><td className="px-3 py-3 max-w-[240px] truncate text-text-pg-dim">{item.news_snippet || "-"}</td></tr>)}</tbody></table>{!earnings.candidates.length ? <p className="p-4 text-sm text-text-pg-muted">{zh ? "暂无财报候选。数据每2日自动刷新。" : "No earnings candidates. Data auto-refreshes every 2 days."}</p> : null}</div>
    </ResearchCard>
