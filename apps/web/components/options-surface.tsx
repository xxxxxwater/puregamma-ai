...
import { Badge, EmptyState, LoadingSkeleton, ResearchCard } from "@/components/puregamma";
...
export function OptionsSurface({ locale, initialCurrency = "BTC" }: { locale: Locale; initialCurrency?: string }) {
  ...
  const [currency, setCurrency] = useState(() => {
    ...
    return param && ["BTC", "ETH", "SOL"].includes(param.toUpperCase()) ? param.toUpperCase() : initialCurrency;
  });
  const [surfaceType, setSurfaceType] = useState<SurfaceType>(() => {
    if (typeof window === "undefined") return "mark_iv";
    const param = new URLSearchParams(window.location.search).get("type");
    return (SURFACE_TYPES as string[]).includes(param || "") ? (param as SurfaceType) : "mark_iv";
  });
  ...
  useEffect(() => {
    ...
    params.set("type", surfaceType);
    window.history.replaceState(null, "", `${window.location.pathname}?${params.toString()}`);
  }, [currency, surfaceType]);
  ...
  const exportCsv = () => {
    if (!data || !data.surface.rows.length) return;
    const header = "S/K,DTE,Value,Strike,Expiry,Instrument,OI,Volume,Type";
    const lines = data.surface.rows.map((row) => [row.x, row.y, row.z, row.strike, row.expiry, row.instrument, row.open_interest, row.volume_24h, row.option_type].join(","));
    const blob = new Blob([[header, ...lines].join("\n")], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `options-surface-${currency}-${surfaceType}.csv`;
  ...
  const askAgent = () => {
    const prompt = zh
      ? `分析 ${currency} 期权 ${surfaceLabel(copy, surfaceType)} 曲面：解读当前形态、关键机会与风险。`
      : `Analyze the ${currency} options ${surfaceType} surface: interpret the current shape, key opportunities, and risks.`;
    window.location.href = withLocale(locale, `/chat?q=${encodeURIComponent(prompt)}`);
  };
  ...
  return (
    <ResearchCard>
      ...
      {!loading && data && (!data.surface.rows.length || data.error) ? (
        <div className="mt-4">
          <EmptyState title={data.error ? copy.emptyErrorTitle : copy.emptyTitle} description={data.error || copy.emptyDescription} />
          <button type="button" onClick={() => void load(currency, surfaceType)} className="mt-3 inline-flex items-center gap-1.5 border border-border-pg px-3 py-1.5 text-xs text-text-pg-muted hover:border-border-pg-strong hover:text-text-pg"><RefreshCw className="h-3 w-3" />{zh ? "重试" : "Retry"}</button>
        </div>
      ) : null}
      ...
      {!loading && data && data.surface.rows.length ? (
        <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 border border-border-pg bg-bg-panel-muted p-3 text-xs">
          ...
          <span className="ml-auto flex items-center gap-2"><button type="button" disabled className="inline-flex items-center gap-1 border border-border-pg px-2 py-1 text-text-pg-dim opacity-50" title={`${copy.insights.alertTitle}${zh ? "（即将推出）" : " (coming soon)"}`}><Bell className="h-3 w-3" />{copy.insights.alert}</button><button type="button" onClick={exportCsv} className="inline-flex items-center gap-1 border border-border-pg px-2 py-1 text-text-pg-muted hover:border-border-pg-strong" title={copy.insights.exportTitle}><Download className="h-3 w-3" />{copy.insights.export}</button><button type="button" onClick={askAgent} className="inline-flex items-center gap-1 border border-border-pg px-2 py-1 text-text-pg-muted hover:border-border-pg-strong" title={copy.insights.agentTitle}><MessageCircleQuestion className="h-3 w-3" />{copy.insights.askAgent}</button></span>
        </div>
      ) : null}
