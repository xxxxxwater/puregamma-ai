    ...
    <div className="mb-8 border-b border-border-pg pb-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="max-w-4xl">
          ...
          <h1 className="max-w-5xl text-3xl font-semibold tracking-normal text-text-pg md:text-5xl">{title}</h1>
          {body ? <p className="mt-4 max-w-3xl text-sm leading-6 text-text-pg-muted md:text-base">{body}</p> : null}
        </div>
...
export function DisclaimerFooter({ locale = defaultLocale }: { locale?: Locale }) {
  const footer = t(locale, "compliance.footer");
  if (!footer) return null;
  return <footer className="mt-10 border-t border-border-pg pt-5 text-xs leading-5 text-text-pg-muted">{footer}</footer>;
}
