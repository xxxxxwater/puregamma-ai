  ...
  return <div className="space-y-4">
    <ResearchCard>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Field label={zh ? "渠道" : "Channel"}><select value={value.channel} onChange={(event) => set("channel", event.target.value as DailyPushPreference["channel"])} className="control">{channels.map((channel) => <option key={channel}>{channel}</option>)}</select></Field>
        <Field label={zh ? "时区" : "Timezone"}><input value={value.timezone} onChange={(event) => set("timezone", event.target.value)} className="control" placeholder="Asia/Shanghai" /></Field>
        <Field label={zh ? "本地时间" : "Local time"}><input type="time" value={value.local_time} onChange={(event) => set("local_time", event.target.value)} className="control" /></Field>
      </div>
      ...
      {value.channel === "imessage" ? <div className="mt-4 grid gap-3 border border-border-pg p-4 md:grid-cols-[1fr_auto_140px_auto]"><input value={value.recipient || ""} onChange={(event) => set("recipient", event.target.value)} className="control" placeholder="+15555550100" /><button type="button" onClick={requestCode} disabled={Boolean(busy)} className="border border-border-pg px-3 py-2 text-sm">{zh ? "发送验证码" : "Send code"}</button><input value={code} onChange={(event) => setCode(event.target.value)} className="control" inputMode="numeric" maxLength={6} placeholder="000000" /><button type="button" onClick={confirmCode} disabled={Boolean(busy) || !challenge} className="border border-border-pg px-3 py-2 text-sm">{zh ? "确认" : "Verify"}</button><p className="text-xs text-text-pg-muted md:col-span-4">{value.recipient_verified_at ? (zh ? "已验证" : "Verified") : (zh ? "尚未验证" : "Not verified")}</p></div> : null}
