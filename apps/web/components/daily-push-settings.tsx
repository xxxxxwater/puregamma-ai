...
export function DailyPushSettings({ initial, initialHistory, locale, plan }: { initial: DailyPushPreference; initialHistory: DeliveryRecord[]; locale: Locale; plan: string }) {
  ...
  return <div className="space-y-4">
    <ResearchCard>
      ...
      {value.channel === "imessage" ? <div className="mt-4 grid gap-3 border border-border-pg p-4 md:grid-cols-[1fr_auto_140px_auto]"><Input value={value.recipient || ""} onChange={(event) => set("recipient", event.target.value)} placeholder="+15555550100" /><button type="button" onClick={requestCode} disabled={Boolean(busy)} className="border border-border-pg px-3 py-2 text-sm">{zh ? "发送验证码" : "Send code"}</button><Input value={code} onChange={(event) => setCode(event.target.value)} inputMode="numeric" maxLength={6} placeholder="000000" /><button type="button" onClick={confirmCode} disabled={Boolean(busy) || !challenge} className="border border-border-pg px-3 py-2 text-sm">{zh ? "确认" : "Verify"}</button><p className="text-xs text-text-pg-muted md:col-span-4">{value.recipient_verified_at ? (zh ? "已验证" : "Verified") : (zh ? "尚未验证" : "Not verified")}</p></div> : null}
