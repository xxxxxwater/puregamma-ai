...
export function HyperliquidMarketPanel({ locale }: { locale: Locale }) {
  ...
  const copy = locale === "zh"
    ? { title: "Autopilot 实时市场观察", subtitle: "Hyperliquid · 15 分钟 K 线 · 只读公开行情", collapse: "收起行情列表", expand: "展开行情列表", price: "最新", change: "15 分钟", volume: "24h 成交额", oi: "未平仓", funding: "资金费率", updated: "UTC" }
    : { title: "Autopilot live market watch", subtitle: "Hyperliquid · 15-minute candles · public read-only feed", collapse: "Collapse market list", expand: "Expand market list", price: "Last", change: "15m", volume: "24h notional", oi: "Open interest", funding: "Funding", updated: "UTC" };
        ...
        <table className="w-full min-w-[980px] font-mono text-xs">
          ...
          <tbody>
            {rows.map(({ instrument, candle, context, last, change, openInterestUsd }) => <tr key={instrument.id} className="border-t border-border-pg/70 hover:bg-bg-panel-muted">
              ...
              <td className="px-3 py-3 text-right text-text-pg">{price(last)}</td>
              <td className={`px-3 py-3 text-right ${change === undefined ? "text-text-pg-dim" : change >= 0 ? "text-status-positive" : "text-status-negative"}`}>{percent(change)}</td>
