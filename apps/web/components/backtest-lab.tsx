...
export function BacktestLab({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="space-y-6 py-4">
      ...
      {error ? <p className="border border-border-pg bg-bg-panel px-4 py-2.5 text-sm text-status-negative rounded-lg">{error}</p> : null}
      ...
      <section className="border border-border-pg bg-bg-panel p-4 rounded-xl">
        <div className="flex flex-wrap items-center gap-3 text-xs">
          ...
          {(status?.symbols || ["BTC", "ETH"]).map((symbol) => {
            ...
            return (
              <span key={symbol} className="border border-border-pg px-2 py-1 text-text-pg-muted rounded-lg">
          ...
          <button type="button" onClick={handleRefreshData} disabled={refreshing} className="ml-auto inline-flex items-center gap-1.5 border border-border-pg px-2.5 py-1.5 font-medium transition hover:border-border-pg-strong disabled:opacity-50 rounded-lg">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="space-y-4 border border-border-pg bg-bg-panel p-5 rounded-xl">
          <h2 className="text-sm font-semibold">{copy.generate.title}</h2>
          <textarea value={idea} onChange={(event) => setIdea(event.target.value)} rows={3} placeholder={copy.generate.ideaPlaceholder} className="w-full border border-border-pg bg-bg-panel-muted p-3 text-sm outline-none focus:border-border-pg-strong rounded-lg" />
          ...
          <button type="button" onClick={handleGenerate} disabled={generating} className="inline-flex items-center gap-2 border border-border-pg-strong bg-pg-white px-4 py-2 text-sm font-semibold text-pg-black disabled:opacity-50 rounded-lg">
        ...
        <section className="space-y-4 border border-border-pg bg-bg-panel p-5 rounded-xl">
          ...
          <textarea value={specText} onChange={(event) => setSpecText(event.target.value)} rows={10} spellCheck={false} className="w-full border border-border-pg bg-bg-panel-muted p-3 font-mono text-[11px] leading-4 outline-none focus:border-border-pg-strong rounded-lg" />
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs">
            ...
            <button type="button" onClick={handleRun} disabled={!parsedSpec || running} className="ml-auto inline-flex items-center gap-2 border border-border-pg-strong bg-pg-white px-4 py-2 text-sm font-semibold text-pg-black disabled:opacity-50 rounded-lg">
      ...
      {selected && perf ? (
        <section className="space-y-5 border border-border-pg bg-bg-panel p-5 rounded-xl">
          ...
          <div className="grid grid-cols-2 gap-px border border-border-pg bg-border-pg md:grid-cols-4 rounded-xl overflow-hidden">
            {[
          ...
          <div className="flex gap-2 text-xs"><button type="button" onClick={() => handleExport("json")} disabled={exporting || selected.status !== "completed"} className="inline-flex items-center gap-1 border border-border-pg px-2.5 py-1.5 hover:border-border-pg-strong disabled:opacity-50 rounded-lg"><Download className="h-3 w-3" /> JSON · 50 credits</button><button type="button" onClick={() => handleExport("csv")} disabled={exporting || selected.status !== "completed"} className="border border-border-pg px-2.5 py-1.5 hover:border-border-pg-strong disabled:opacity-50 rounded-lg">CSV · 50 credits</button></div>
          ...
          {selected.trades?.length ? (
            <details className="text-xs text-text-pg-muted">
              ...
              <pre className="mt-2 max-h-64 overflow-auto border border-border-pg bg-bg-panel-muted p-3 text-[11px] leading-4 rounded-xl">{JSON.stringify(selected.trades, null, 2)}</pre>
            </details>
          ) : null}
          {selected.positions?.length ? (
            <details className="text-xs text-text-pg-muted">
              <summary className="cursor-pointer font-medium">Position records ({selected.positions.length})</summary>
              <pre className="mt-2 max-h-64 overflow-auto border border-border-pg bg-bg-panel-muted p-3 text-[11px] leading-4 rounded-xl">{JSON.stringify(selected.positions, null, 2)}</pre>
            </details>
          ) : null}
          {perf.per_asset ? (
            <div>
              <h3 className="mb-2 text-xs font-semibold text-text-pg-muted">{copy.performance.perAsset}</h3>
              <div className="grid gap-px border border-border-pg bg-border-pg md:grid-cols-2 rounded-xl overflow-hidden">
          ...
          <details className="text-xs text-text-pg-muted">
            ...
            <pre className="mt-2 overflow-x-auto border border-border-pg bg-bg-panel-muted p-3 text-[11px] leading-4 rounded-xl">{JSON.stringify(selected.assumptions, null, 2)}</pre>
          </details>
        </section>
      ) : null}
