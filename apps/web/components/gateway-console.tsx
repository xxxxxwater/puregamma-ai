...
export function GatewayConsole({ locale, dashboard, initialKeys, initialRequests, initialUsage }: Props) {
  ...
  return <div className="space-y-5">
    {dashboard.unavailable ? <div className="border border-status-warning bg-bg-panel p-4 text-sm text-text-pg-muted rounded-xl">{zh ? "Gateway 数据暂不可用。请确认网关已启用并使用已登录账户访问。" : "Gateway data is unavailable. Confirm that the gateway is enabled and you are signed in."}</div> : null}
