  ...
  return <div className="space-y-5">
    ...
    <ResearchCard>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button type="button" onClick={create} disabled={busy || keys.filter((key) => key.status !== "revoked").length >= 10} className="inline-flex items-center gap-2 border border-border-pg bg-pg-white px-3 py-2 text-sm font-semibold text-pg-black disabled:opacity-50"><KeyRound className="h-4 w-4" />{zh ? "创建密钥" : "Create key"}</button>
      </div>
