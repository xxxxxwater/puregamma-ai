...
export function HoldingRow({ holding, zh, money, pct, quantity }: { holding: PortfolioHolding; zh: boolean; money: (value: number) => string; pct: (value: number | null | undefined) => string; quantity: (value: number) => string }) {
  ...
  return <tr>
    <td className="px-4 py-3"><div className="flex items-center gap-2"><span className="font-medium">{holding.symbol}</span>{holding.native ? <span className="border border-border-pg px-1 py-0.5 text-[9px] text-text-pg-dim rounded-lg">NATIVE</span> : null}{!holding.verified ? <span className="border border-status-warning px-1 py-0.5 text-[9px] text-status-warning rounded-lg">{zh ? "未验证" : "UNVERIFIED"}</span> : null}{!holding.priced ? <span className="border border-border-pg px-1 py-0.5 text-[9px] text-text-pg-dim rounded-lg">{zh ? "无报价" : "NO PRICE"}</span> : null}</div><div className="mt-0.5 max-w-40 truncate text-[10px] text-text-pg-dim">{holding.name}</div></td>
    <td className="px-3 py-3">{holding.chain ? <span className="border border-border-pg px-1.5 py-0.5 font-mono text-[10px] uppercase text-text-pg-muted rounded-lg">{holding.chain}</span> : <span className="text-text-pg-dim">--</span>}</td>
