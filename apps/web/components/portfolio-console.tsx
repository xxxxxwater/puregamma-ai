...
import { connectEvmWallet, connectHyperliquid, createEvmWalletChallenge, createPlaidLinkToken, disconnectPortfolioAccount, exchangePlaidToken, getMe, getPortfolioAutopilot, getPortfolioSnapshot, getSkillCatalog, runPortfolioAutopilot, syncPortfolioAccount, updatePortfolioAutopilot, type PortfolioAutopilot, type PortfolioHolding, type PortfolioSnapshot, type SkillSummary } from "@/lib/api";
...
export function PortfolioConsole({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      if (document.visibilityState === "visible") void getPortfolioSnapshot(locale).then(setPortfolio).catch(() => undefined);
    }, 60_000);
  ...
  const sync = async (id: string) => { setBusy(id); setError(""); try { setPortfolio(await syncPortfolioAccount(id)); } catch (reason) { setError(formatError(reason)); } finally { setBusy(""); } };
  const syncAll = async () => {
...
function HoldingRow({ holding, zh, money, pct, quantity }: { holding: PortfolioHolding; zh: boolean; money: (value: number) => string; pct: (value: number | null | undefined) => string; quantity: (value: number) => string }) {
