...
@MainActor @Observable final class TodayViewModel {
    ...
    func load() async {
        ...
        market = Self.cachedState(assets); reports = Self.cachedState(reportRows); billing = Self.state(summary)
    }
    private static func cachedState<T>(_ result: Result<CachedRepositoryValue<T>, Error>) -> LoadState<T> { switch result { case .success(let result): if let rows = result.value as? any Collection, rows.isEmpty { return .empty }; return result.cachedAt.map { .stale(result.value, $0) } ?? .loaded(result.value); case .failure(let error): return .failed(error as? APIError ?? .transport(error.localizedDescription)) } }
    private static func state<T>(_ result: Result<T, Error>) -> LoadState<T> { switch result { case .success(let value): if let rows = value as? any Collection, rows.isEmpty { return .empty }; return .loaded(value); case .failure(let error): return .failed(error as? APIError ?? .transport(error.localizedDescription)) } }
}
