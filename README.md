...
is a point-in-time record, not the current state.
...
What the trunk contains today:

- **PureGamma Harness plugin runtime (in progress)**: `harness/` holds 58 plugin
  packages — service definitions, legacy compatibility providers, model-facing
  tools, workers and UI contributions — with the machine-readable catalog in
  `harness/plugins/catalog.ts`. The target is a DeepSeek Harness profile powered
  by Cordis, where business capabilities become plugins rather than a privileged
  application core. See [Harness Core V2](./docs/architecture/HARNESS_CORE_V2.md)
  for the decision, the migration slices and the per-capability retirement gate.
...
- Mobile backend status: `/api/mobile/capabilities` (`apps/api/routers/mobile.py`), `/api/research/runs*` (`apps/api/routers/harness_runs.py`) and `/api/memory/*` (`apps/api/routers/memory.py`) **are implemented**; `/api/trading/mandates*` plus `/api/trading/connections`, `/api/trading/orders/*`, and `/api/trading/safety-status` **are implemented** by the LIVE trading control plane push. The client still renders unavailable states honestly whenever a capability boolean is false.
...
- Partially implemented or placeholder: real-time market stream parity, some P2+ internal surfaces (Risk/Realtime/MCP contracts return `NOT_IMPLEMENTED`), Redis Streams event pipeline and DLQ, Bloomberg enterprise import, and tenant/workspace isolation (there is no `tenant_id`; isolation is per-user).
- Planned or documented contract: production launch gating, App Store/Play release, real broker adapter provisioning for LIVE (`LIVE_TRADING_GATEWAY` defaults to `mock`; `nautilus` and `binance` exist as code paths, and `binance` requires `LIVE_TRADING_PROVIDER=binance_spot` — but no production broker connection is approved), first-user LIVE approvals.
...
```
apps/
  api/                 FastAPI backend (42 routers, services)
  web/                 Next.js web app (en/zh)
  ios/                 SwiftUI iOS app
  android/             Kotlin/Compose Android app
  pocket-relay/        Mobile access relay (cloudflared tunnel + QR pairing)
...
packages/
  ...
  database/            Models + Alembic migrations (head 0032_chat_workspace)
  decisions/           JEV advisory: intent, redaction, rerank, client
  gateway/             AI gateway catalog, pricing, security, usage
  harness/             Deep-research orchestration + security gates
  live_trading/        LIVE control plane: gates, risk, ledger, NAV, kill switches, reconciliation, gateway adapter
...
harness/               PureGamma Harness workspace: Cordis profile, 58 plugin packages, plugins/catalog.ts
...
- LIVE rollout (only after every gate passes — see `docs/live-trading/`): provision a real broker adapter (`LIVE_TRADING_GATEWAY=nautilus`, or `binance` with `LIVE_TRADING_PROVIDER=binance_spot`), approve a small pilot cohort (`/admin/trading/live-approvals`), approve production-environment mandates, then flip the deployment marker and `LIVE_TRADING_ENABLED` last.
- Ship the iOS and Android apps to the App Store and Play (code complete, release pending).
