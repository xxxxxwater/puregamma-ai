...
Backend NAV is implemented: portfolio accounts, positions, transactions, prices, and NAV snapshots are synced and persisted by `portfolio_service`. Supported sources are Plaid Investments, EVM wallets (via Moralis), Hyperliquid, Interactive Brokers, and private CEX read-only keys (Binance, OKX, Bybit). A scheduled worker job (`puregamma.sync_all_portfolio_accounts`) refreshes every connected portfolio account on a 15-minute cadence, and Autopilot-gated sync/review is handled separately (`puregamma.sync_portfolio_autopilot_accounts`). Snapshots are exposed through `portfolio_view` and `portfolio_context`.
